...
# If there is no peak in the given array, then the output should be
#  {pos: [], peaks: []}.
...
# plateau. For example: pickPeaks([1, 2, 2, 2, 1]) returns {pos: [1], peaks: [2]} 
...
def pick_peaks(arr):
    ...
    for i in range(1, len(arr)):
        if arr[i] > arr[i-1]:
            prob_peak = i
        elif arr[i] < arr[i-1] and prob_peak: #a number is True
...
# [1] + [2] gives you a single array [1,2]
